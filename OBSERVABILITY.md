...
- **No third-party analytics SDK.** At this scale (single user), a third-party analytics/event-tracking SDK (Firebase, Amplitude, etc.) adds a new vendor dependency and a new place sensitive-adjacent usage data could leak, for a benefit this project can already get in-house. Product analytics live in Supabase itself (§4).
- **Native tooling first.** `OSLog` and `MetricKit` (Apple, on-device/privacy-respecting by default) are the first choice for diagnostics before reaching for anything external — consistent with `ARCHITECTURE.md` §14's original decision, expanded here.
...
`OSLog` categories, each with standard levels (`debug` / `info` / `error` / `fault`):
...
Debug-level logs are compiled out of release builds; `info`/`error`/`fault` persist via `OSLog` for on-device inspection (Console.app) and, for `error`/`fault` in the `Sync` and `AI` categories specifically, also forwarded to the backend (§5) so failures are visible without needing physical access to the device.
...
- Background task (`BGTaskScheduler`) success/failure and completion latency.
...
- **Primary**: `MetricKit` (Apple-native, on-device diagnostics, privacy-respecting, no third-party SDK) — crash reports, hangs, disk writes, launch metrics.
- **Supplement**: explicit error logging (§2, `error`/`fault` levels) from `catch` blocks in critical paths (`SyncCoordinator`, AI engine calls) sent to the backend immediately — `MetricKit` payloads can be delayed by hours/days, and a sync or AI-call failure needs faster visibility than that for a solo maintainer to act on.
...
- Battery/memory spot-checks (manual, Instruments-driven — not continuously instrumented in production, per §1's operational-not-surveillance principle and iOS platform constraints on runtime battery introspection).
...
- A **dev-only, debug-build-flagged SwiftUI "Health" screen** in the app itself: last sync status, last successful extraction/profile-update job, this month's token spend, recent error log tail. Cheap, native, always available without separate infrastructure.
...
- `ARCHITECTURE.md` §14 states the tooling *choice* (OSLog/MetricKit); this document is the detailed operational specification built on that choice.

...
**Status: implementation started.** Macro-stages 1–4 are closed: project skeleton, Presentation folder structure + file-header convention, the monorepo workspace (`@coach/domain`/`@coach/data`, boundary enforcement via ESLint — a real gap in ADR-021's original module-resolution claim was found and corrected along the way, see ADR-021's 2026-08-10 note), and local persistence (`expo-sqlite` + Drizzle, real generated migrations, 21 passing tests). One honest open item carried forward from macro-stage 4: `createProductionDb`'s actual on-device/web runtime behavior is unverified (needs a physical device, or a decision on whether web needs local SQLite at all — see task 4.3).

Macro-stage 5 (Supabase) is partially done (2026-08-11): Julia provisioned a real project and provided the URL + publishable key. Tasks 5.1/5.2/5.5 are code-complete and locally verified as far as this session's network policy allows (direct access to `*.supabase.co` is blocked by this session's egress policy — not a code problem, reported rather than routed around, per this environment's own proxy guidance). The schema+RLS SQL (5.2) is handed to Julia to run herself in the Supabase SQL Editor. Tasks 5.3/5.4 genuinely need live dashboard/CLI reachability this session doesn't have — not attempted blind. Macro-stages 6+ remain open, several depending on 5.2 actually being applied first.
...
- **5.1 — Status: ✅ Done (2026-08-11).** Provision the Supabase project, enable `pgvector`. *(Unaffected. Execution Environment: Any Environment.)* **Execução real:** Julia created the project herself (`wsfrvhpnuaglzukobpxi`, region `ca-central-1`) — this is an account-holder action, not something I can do on her behalf. `create extension if not exists vector;` is the first line of `supabase/migrations/0001_initial_schema.sql` (5.2), so enabling `pgvector` and creating the schema happen in the same one-time script run rather than as two separate steps.
- **5.2 — Status: ✅ Done (2026-08-11), code-complete; not yet applied to the live project — see below.** Write and apply the initial schema migration (`ARCHITECTURE.md` §9.1 — tables, indexes, RLS). *(Unaffected. Execution Environment: Any Environment.)* **Execução real:** `supabase/migrations/0001_initial_schema.sql` — the nine tables and four indexes are copied verbatim from `ARCHITECTURE.md` §9.1. The RLS policies are new work: §9.1 never specified them in detail, only `ARCHITECTURE_DECISIONS.md`'s approval checklist named the *intent* ("Defensive per-user Row Level Security despite single-user v1 scope"). Designed here following Supabase's standard per-user pattern — every table enables RLS, with a single `for all using (auth.uid() = user_id) with check (...)` policy (or `= id` on `users` itself). The `streaks` view is deliberately excluded: §9.1 itself marks its real query as "implementation detail for macro-stage 8," and the placeholder shown there produces meaningless data — added for real in macro-stage 8, not faked now. **Not yet applied**: this session's network egress policy blocks direct access to `*.supabase.co` (confirmed via a real connection attempt, "Host not in allowlist" — not a code problem, an organizational network policy for this session, per this environment's own proxy documentation: "do not retry or route around it, report the blocked host"). The SQL is handed to Julia to paste into the Supabase SQL Editor herself.
- **5.3 — Status: 🔲 Blocked on Julia's action (2026-08-11).** Configure Auth and Storage (test user, `transcripts/` bucket policy, positive/negative access tests). *(Unaffected. Execution Environment: Any Environment.)* Genuinely needs live dashboard/CLI access to the project this session's network policy blocks — not started.
- **5.4 — Status: 🔲 Blocked on Julia's action (2026-08-11).** Scaffold the Edge Functions deploy pipeline with a trivial `hello` function. *(Unaffected. Execution Environment: Any Environment.)* Same blocker as 5.3 — deploying to a live project needs reachability this session doesn't have.
- **5.5 — Status: ✅ Done (2026-08-11), code + build verified; live-project round-trip unverified for the same network-policy reason as 5.2.** Wire the Supabase JS SDK with safe secret handling. **Objetivo:** add `@supabase/supabase-js` to `packages/core/data`, configure the client from Expo's environment-variable mechanism (`app.config.ts` + `.env` files, **not** committed) rather than a Swift `.xcconfig` file. **Criar:** `packages/core/data/src/supabase/supabaseClientFactory.ts`, `.env.example` (committed template). **Modificar:** `.gitignore` (confirm `.env` — not `.env.example` — is ignored). **Depende de:** 5.1, macro-stage 3. **Critérios de conclusão:** a debug run successfully calls the live project; `git status` after adding real secrets shows `.env` untracked. **Execução real:** `createSupabaseClient(url, anonKey)` takes both as parameters (ADR-012, mirrors `dbFactory.ts`'s pattern from 4.3), not read from `process.env` internally — that stays at the composition root once it exists. `apps/mobile/.env` (gitignored, confirmed via `git check-ignore`) holds Julia's real Project URL + publishable key; `.env.example` is the committed template. **Two real findings, both fixed:** (1) the session-storage adapter (needed since Supabase's JS client defaults to `localStorage`, which doesn't exist in React Native) initially used `expo-secure-store` unconditionally — its native module has no web implementation at all and threw `getValueWithKeyAsync is not a function` on the web target, caught via a temporary (reverted) wiring into `apps/mobile/app/index.tsx` and a real bundled-app screenshot, not assumed; fixed with a `typeof localStorage`-based branch to a `localStorage`-backed adapter on web (the accepted-weaker fallback ADR-024 already named) and `expo-secure-store` on iOS/Android. (2) Even after that fix, `expo-secure-store`'s own import chain (via `expo-modules-core`'s `requireNativeModule`) pulls in `react-native` at module-load time regardless of which branch runs, breaking this package's own test suite outside Metro (`react-native`'s source uses Flow syntax Vitest's Rolldown pipeline can't parse) — same class of problem as `dbFactory.ts`'s static `expo-sqlite` import in task 4.3. The same fix (deferring the import) turned out to be necessary but not sufficient here: `@supabase/supabase-js`'s `createClient()` calls `storage.getItem(...)` internally during construction (checking for a persisted session), so the dynamic `expo-secure-store` import still fired during test runs — just asynchronously, surfacing as three unhandled-rejection `EnvironmentTeardownError`s *after* all 23 tests had already reported green, not as a visible failure (`npm test`'s exit code caught it; a quick glance at "tests passed" would not have). The actual fix: `vitest.config.ts` aliases `expo-secure-store` to a lightweight test-only stub (`test/__mocks__/expo-secure-store.ts`) — Metro/the real Expo runtime still resolves the genuine package normally; only this test suite ever sees the stub. **Riscos:** the same highest-risk-in-this-stage accidental-secret-commit concern as the original task, now specifically about `.env` files rather than `.xcconfig` — Expo's public `EXPO_PUBLIC_*` env var prefix convention is a common source of accidental exposure (anything prefixed that way IS bundled into the client, by design) and needs explicit care: **only the Supabase anon key** (safe by Supabase's own design to be public) uses that prefix; nothing else does. **Testes:** unit test (`supabaseClientFactory.test.ts`) proves the factory builds a client from its actual parameters, not a hardcoded default, and that separate calls don't share a singleton (ADR-012) — no network involved, works regardless of this session's egress policy. Real-build verification: `tsc` clean, `expo start --web` bundled and served, and a headless-Chromium screenshot of the temporary wiring showed `createSupabaseClient() built + getSession() call succeeded` — `auth.getSession()` on a fresh client resolves locally without a network round-trip, so this exercises the real SDK/storage-adapter code path even under this session's network restriction; it does not prove connectivity to the live project (that needs 5.2 applied first, then a check from an unrestricted network — Julia's own device, or a future session with different egress policy). Not a naming collision but a real overlap worth flagging for macro-stage 6.2, which already names `packages/core/data/src/security/secureSessionStore.ts` for the broader `AuthRepository`'s session storage: this task's `supabase/secureStoreAdapter.ts` solves the identical underlying problem (where does the Supabase session token live, web vs native) a macro-stage early. 6.2 should reconcile with — reuse or explicitly supersede — what 5.5 already built here, not design a second, parallel answer to the same question. **Impacto na arquitetura:** implements `ARCHITECTURE.md` §7. **Execution Environment: Any Environment** for everything except the live-project round-trip, which needs a network reachable to `*.supabase.co`.

**Macro-stage exit criteria:** unaffected — live Supabase project with full schema, RLS, Storage, Auth smoke-tested; client SDK wired with no secret committed. **Partially met (2026-08-11):** 5.1/5.2/5.5 are code-complete and locally verified as far as this session's network policy allows; 5.2's SQL is handed to Julia to run herself; 5.3/5.4 are genuinely blocked pending either Julia's own dashboard/CLI action or a session with different network reachability — not attempted blind.

...
React Native + Expo, TypeScript (strict), Expo Router, `expo-sqlite` + Drizzle ORM (local cache), Supabase (Postgres + `pgvector` + Auth + Edge Functions + Storage, source of truth), Groq (`llama-3.3-70b-versatile`, free tier — reasoning/content) + browser `SpeechRecognition`/`expo-speech` (voice), MVVM + Clean Architecture, monorepo workspace packages (`@coach/domain` / `@coach/data`), Expo Go (no local macOS dependency, no Apple Developer Program fee). **Zero-cost constraint (2026-08-10, ADR-025/026):** web/PWA (browser on the notebook) is the primary surface for voice; iOS/Android via Expo Go is text-only, no custom dev-client build, no EAS/TestFlight distribution — conditional on this constraint, not permanent (`ARCHITECTURE_DECISIONS.md` ADR-026). Full rationale in `ARCHITECTURE.md` and `ARCHITECTURE_DECISIONS.md`.

...
> **2026-08-10 correction (found while executing `IMPLEMENTATION_PLAN.md` macro-stage 3.2 — the decision is not superseded, one factual claim above is wrong as built):** "a domain file literally cannot resolve an import to a package it doesn't depend on" does **not** hold for npm workspaces. Confirmed empirically: `packages/core/domain` importing `@coach/data` directly resolves with zero error at both the `tsc` and Node module-resolution level, because npm hoists every workspace package into the shared root `node_modules/@coach/*` and neither TypeScript nor Node consults a package's own `dependencies` field to gate what it's *allowed* to import — only whether the target resolves at all. The dependency-graph mechanism this ADR describes as the primary enforcement (with the lint rule as "defense-in-depth") only actually blocks packages that are entirely absent from the monorepo's install (e.g. `@supabase/supabase-js`, not yet installed anywhere) — it does **not** block a sibling workspace package. Decision and Justification above are otherwise unchanged: the monorepo-package structure still directly serves ADR-013's cross-platform-reuse goal regardless of this correction. **The `eslint-plugin-boundaries` rule (`eslint.config.mjs`) is the actual primary enforcement for the domain↔data direction, not defense-in-depth as originally written** — see `IMPLEMENTATION_PLAN.md` macro-stage 3.2/3.3 for the full account, including two further API details (`checkAllOrigins`, `module.source` vs `element.type` matching) that were also found empirically rather than assumed from a remembered API version.

...
- **D2 — AI engine**: ~~Hybrid, decoupled by design. Anthropic Claude is the primary engine for conversation, explanations, corrections, and content/lesson generation. OpenAI Realtime API handles speech-to-text/text-to-speech for live voice conversation.~~ **Superseded 2026-08-10 (ADR-025, ADR-026), driven by D4's supersession below:** conversation/reasoning now runs on **Groq (`llama-3.3-70b-versatile`, free tier)** — chosen over the larger-quota Gemini free tier specifically because Groq doesn't train on user data (`RISKS.md` R-09's sensitivity concern). Voice input runs on the **browser's native `SpeechRecognition` API** on the web/PWA target (now the primary voice surface, ADR-026), not OpenAI Realtime — the OpenAI Realtime leg is dropped entirely, it has no free tier. The Domain-layer interface abstraction itself (`ConversationEngine`, `VoiceEngine`) is unchanged and unsuperseded — this is exactly the adapter swap that abstraction was built for (ADR-002/ADR-005), not evidence it needs rework. See `ARCHITECTURE.md` §5.
...
- **D4 — Budget posture**: ~~Moderate. Willing to pay for a high-quality experience, but the architecture must actively avoid unnecessary API calls (caching, batching, on-device alternatives where sensible) and must keep providers swappable so cheaper or better alternatives can be adopted later without a rewrite.~~ **Superseded 2026-08-10 (ADR-025, ADR-026):** **zero cost, absolute, no exception** — confirmed directly by Julia after seeing the app's actual state (a placeholder screen) and deciding to move to real implementation. Every AI/voice/distribution choice from this point on is filtered through this constraint first, quality second — the opposite priority order from the original D4. Provider-swappability (the one part of the original D4 that survives unchanged) is what makes reverting to a paid stack later, if this constraint ever lifts, a targeted adapter swap rather than a rewrite.
